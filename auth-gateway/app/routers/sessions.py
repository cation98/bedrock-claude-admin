...
async def create_session(
    ...
    if existing:
        # K8s 실제 상태와 동기화 후 반환
        if existing.pod_name:
            pod_info = k8s.get_pod_status(existing.pod_name)
            if pod_info and pod_info["phase"] == "Running":
                existing.pod_status = "running"
                db.commit()
            elif pod_info and pod_info["phase"] in ("Failed", "Succeeded"):
                # Pod 종료됨 (TTL 만료, crash, FailedMount 등) — terminated 처리 후 새로 생성
                logger.warning(
                    "session reconcile: user=%s pod=%s k8s_phase=%s -> terminated (create_session)",
                    username, existing.pod_name, pod_info["phase"],
                )
                ...
                logger.warning(
                    "session reconcile: user=%s pod=%s k8s=disappeared -> terminated (create_session)",
                    username, existing.pod_name,
                )
                existing.pod_status = "terminated"
                existing.terminated_at = datetime.now(timezone.utc)
...
async def list_my_sessions(
    ...
    # 활성(creating/running) 세션의 Pod 상태를 K8s에서 동기화
    for session in sessions:
        if session.pod_status in ("creating", "running") and session.pod_name:
            ...
            if pod_status and pod_status["phase"] == "Running":
                ...
                logger.warning(
                    "session reconcile: user=%s pod=%s k8s_phase=%s -> terminated (list)",
                    session.username, session.pod_name, pod_status["phase"],
                )
                ...
                # Pod이 K8s에서 사라진 경우 — terminated 처리
                logger.warning(
                    "session reconcile: user=%s pod=%s k8s=disappeared -> terminated (list)",
